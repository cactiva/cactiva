...
export class CactivaEditorWidget extends CodeEditorWidget {
	...
	constructor(
		...
		(async () => {
			...
			if (!CactivaWorker.worker) {
				const url = getWorkerBootstrapUrl(require.toUrl('./cactiva/cactiva-worker.txt'), (new Date()).getTime().toString());
	...
	protected async _attachModel(model: ITextModel | null): Promise<void> {
		...
		if (!!model) {
			...
			this._modelData?.listenersToRemove.push(
				this.onDidChangeCursorPosition(this._cactivaOnDidChangeCursorPosition.bind(this))
			);
			this._modelData?.listenersToRemove.push(
				this.onDidChangeModelContent(this._cactivaOnDidChangeModelContent.bind(this))
			);
			if (this._splitView.length === 1) {
				this.superLayout({
					width: this._domEl.clientWidth,
