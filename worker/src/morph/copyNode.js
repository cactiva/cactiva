...
export function copyNode(source, fromPath, toPath, position, count) {
  if (!source) return "";
  ...
  if (
    ...
    if (position === "children") {
      if (to instanceof JsxElement || to instanceof JsxFragment) {
        ...
        to.replaceWithText(
          `${toTagUnclosed}>${code}</${to.getTagNameNode().getText()}>`
        );
      }
      ...
      if (parent) {
        ...
        if (children) {
          ...
          if (toIndex >= 0) {
            const childrenText = children.map((e) => e.getText().trim());
            childrenText.splice(
    ...
    return copyNode(source, fromPath, toPath, position, (count || 0) + 1);
  }
}
